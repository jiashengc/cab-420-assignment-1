...
while (~done) 
  ...
  Jsur(iter) = mean((log(logistic(obj, X)) .* -Y) - (log(1 - logistic(obj, X)) .* (1 - Y)) + reg * sum((obj.wts * obj.wts')'));
  ...
  JD = mean(-Y .* log(logistic(obj, X)) - (1-Y) .* log(1 - logistic(obj, X)) + reg * obj.wts * obj.wts');
